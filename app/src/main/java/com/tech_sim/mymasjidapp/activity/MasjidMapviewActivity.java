...
public class MasjidMapviewActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        masjiidmodels= PreferenceManager.masjiidmodels;
        ...
        myRecyclerViewAdapter.setClickListener(new MyRecyclerViewAdapter.ItemClickListener() {
            ...
            public void onMasjidName(String name,String id,String city) {
                Constants.masid_name=name;
                Constants.masjid_id=id;
                PreferenceManager.setMasjid(id);
                Constants.city=city;
                PreferenceManager.setCity(city);
                PreferenceManager.setMasjidCity(city);
                Intent intent=new Intent(MasjidMapviewActivity.this,TrialRegister.class);
                startActivity(intent);
                finish();
                PreferenceManager.masjiidmodels=new ArrayList<>();
            }
        });
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        int count=PreferenceManager.masjiidmodels.size();
        for(int i=0;i<count;i++) {
            float latitude =Float.parseFloat(PreferenceManager.masjiidmodels.get(i).getLatitude());
            float longitude = Float.parseFloat(PreferenceManager.masjiidmodels.get(i).getLongitude());
            String name=PreferenceManager.masjiidmodels.get(i).getName();
